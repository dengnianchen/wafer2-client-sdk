...
function request(options) {
	if (typeof options !== 'object') {
		...
		throw new RequestError(0, 'INVALID_PARAM', message);
	}
	...
	*/
	...
	function doRequestWithLogin() {
		...
		loginLib.loginWithCode({
			success: doRequest,
			fail: function(e) {
				callFail(new RequestError(0, 'REQUEST_ERROR', e.message));
			},
		});
	}
		...
		wx.request(utils.extend({}, options, {
			...
			success: function(response) {
				...
				if (!data || data.code == null) {
					return callFail(new RequestError(0, 'INVALID_RESPONSE_FORMAT',
			...
			fail: function(err) {
				callFail(new RequestError(0, 'REQUEST_ERROR', err.msg));
			},
			complete: noop,
		}));
	}
